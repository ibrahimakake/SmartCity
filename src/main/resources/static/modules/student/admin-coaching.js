...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadData() {
        try {
            entities = await coachingApi.getAll();
            renderTable();
        } catch (e) {
            console.error(e);
    ...
    function renderTable() {
        ...
        els.table.innerHTML = '';
        if (res.length === 0) {
            els.table.innerHTML = `<tr><td colspan="7" class="text-center">No centers found.</td></tr>`;
            return;
        }
        ...
        res.forEach(item => {
            els.table.innerHTML += `
                <tr>
                    ...
                    <td>${item.startingPrice ? item.startingPrice + ' DH' : '-'}</td>
    ...
    function openAdd() {
        ...
        document.getElementById('modalTitle').textContent = 'Add Coaching Center';
        els.form.reset();
    ...
    window.edit = async (id) => {
        ...
        isEditMode.id = id;
        document.getElementById('modalTitle').textContent = 'Edit Coaching Center';
        try {
            const d = await coachingApi.getById(id);
            ...
            document.getElementById('startingPrice').value = d.startingPrice;
    ...
    async function save(e) {
        ...
        try {
            if (isEditMode.active) await coachingApi.update(isEditMode.id, data);
            else await coachingApi.create(data);
            close();
            loadData();
        } catch (e) {
            console.error(e);
