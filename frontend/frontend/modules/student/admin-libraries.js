...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadData() {
        try {
            entities = await librariesApi.getAll();
            renderTable();
        } catch (e) {
            console.error(e);
    ...
    function renderTable() {
        ...
        els.table.innerHTML = '';
        if (res.length === 0) {
            els.table.innerHTML = `<tr><td colspan="7" class="text-center">No libraries found.</td></tr>`;
            return;
        }
        ...
        res.forEach(item => {
            els.table.innerHTML += `
                <tr>
                    ...
                    <td>${item.address}</td>
                    <td>${item.membershipFee ? item.membershipFee + ' DH' : 'Free'}</td>
    ...
    function openAdd() {
        ...
        document.getElementById('modalTitle').textContent = 'Add Library';
        els.form.reset();
    ...
    window.edit = async (id) => {
        ...
        try {
            const d = await librariesApi.getById(id);

            document.getElementById('name').value = d.name;
            document.getElementById('address').value = d.address;
            document.getElementById('contact').value = d.contact;
            document.getElementById('membershipFee').value = d.membershipFee;
    ...
    async function save(e) {
        ...
        try {
            if (isEditMode.active) await librariesApi.update(isEditMode.id, data);
            else await librariesApi.create(data);
            close();
            loadData();
        } catch (e) {
            console.error(e);
